...
namespace SequentialMultiStageSystem
{
    ...
    class DecompositionMethod
    {
        ...
        static int nbJobs;
        static int nbMchs;
        static List<List<int>> M = new List<List<int>>();
        ...
        static private void Input() // функция считывания данных из файла
        {
            using (var file1 = File.Open("Input.txt", FileMode.Open))
            {
                var stream1 = new StreamReader(file1);
                Q = Convert.ToInt32(stream1.ReadLine());
                nbJobs = Convert.ToInt32(stream1.ReadLine());
                nbMchs = Convert.ToInt32(stream1.ReadLine());
                ...
                for (int i = 0; i < nbJobs; i++) // считываем матрицу M (номеров приборов) и T (длительностей выполнения операций)
                {
                    ...
                    int[] ints = line.Split(' ').Select(n => int.Parse(n)).ToArray();

                    for (int j = 0; j < nbMchs; j++)
                    {
                        temp.Add(Convert.ToInt32(ints[j * 2]));
                        temp2.Add(Convert.ToInt32(ints[j * 2 + 1]));
                    }
                    M.Add(temp);
                    T.Add(temp2);
                }
        ...
        static private void ProblemStatement() // ввод ограничений задачи, ввод критерия оптимизации
        {
        ...
        static private int SolveSubproblem() // решение подзадачи в узле дерева решений
        {
            try
            {
                if (cplex.Solve())
        ...
        static private void BranchAndBoundMethod(List<Edge> ordered_edges_q)
        {
            int solution_1, solution_2;

            solution_1 = SolveSubproblem(); // решаем релаксированную подзадачу

            branches = new List<List<Arc>>();
            branches.Add(new List<Arc>());

            records = new List<int>();
            records.Add(solution_1);
            ...
            for (int s = 1; (current_quantity < ordered_edges_count) && (branches.Count() != 0); s++) // основной цикл решателя
            {
                ...
                else
                {
                    if ((solution_1 != 0) && (solution_2 != 0))
                    {
                        ...
                        {
                            if (records.Count() < current_quantity + 2)
                            {
                                records.Add(solution_1);
                            ...
                            {
                                if (records[current_quantity] >= solution_1)
                                {
                                    records[current_quantity] = solution_1;
                                    branches[number_opt].Add(SaveNewArc(ordered_edges_q[current_quantity].tau1, ordered_edges_q[current_quantity].tau2, ordered_edges_q[current_quantity].t2));
                                    solutions[number_opt] = solution_1;
                        ...
                        else
                        {
                            if (solution_1 > solution_2)
                            {
                                ...
                                {
                                    if (records[current_quantity] >= solution_2)
                                    {
                                        records[current_quantity] = solution_2;
                                        branches[number_opt].Add(SaveNewArc(ordered_edges_q[current_quantity].tau2, ordered_edges_q[current_quantity].tau1, ordered_edges_q[current_quantity].t1));
                                        solutions[number_opt] = solution_2; 
                                    }
                                    else
        ...
        static private void Decomposition()
        {
        ...
        static private void GantOutput()
        {
            var file3 = File.Open("OutputGant.txt", FileMode.Open);
            var stream3 = new StreamWriter(file3);

            stream3.Write(nbJobs + "\n");
            stream3.Write(nbMchs + "\n");

            for (int i = 0; i < nbJobs; i++)
            {
                for (int j = 0; j < nbMchs - 1; j++)
                {
                    stream3.Write(M[i][j] + " "); 
                }
                stream3.Write(M[i][nbMchs - 1] + "\n"); 
            }
            ...
            for (int i = 0; i < nbJobs; i++)
            {
                ...
                stream3.Write(T[i][nbMchs - 1] + "\n");
            }

            stream3.Write(cplex.GetValue(tau[0]) + "\n");
            for (int i = 0; i < nbJobs; i++)
            {
                for (int j = 0; j < nbMchs - 1; j++)
                {
                    stream3.Write(cplex.GetValue(tau[i * nbMchs + j + 1]) + " ");
                }
                stream3.Write(cplex.GetValue(tau[i * nbMchs + nbMchs]) + "\n");
            }
            stream3.Write(cplex.GetValue(tau[tau.Count() - 1]) + "\n");
